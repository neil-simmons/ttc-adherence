...
# ==============================================================================
# 4. MATHEMATICAL PIPELINE & VISUALIZATION GENERATOR
# ==============================================================================
def execute_math_pipeline(selected_sig_idx, parquet_path, selected_route, gtfs_route_trips, stop_times, stops, shapes):
    """Executes the tracking logic once a signature is chosen."""
    try:
...
def generate_visuals_and_map():
    """Generates Plotly charts and Kepler config from pipeline data."""
    if not st.session_state.raw_pipeline_data: return
    ...
    for stop in st_filtered.itertuples():
        ...
        hover = (f"<span style='font-size:12px'><b>{stop.stop_name}</b></span><br><br>Sample Size: <b>N = {N}</b><br>Median Arrival: <b>{med:+.1f} min</b><br>Interquartile Range: {q1:+.1f} to {q3:+.1f} min<extra></extra>")

        fig_A.add_trace(go.Violin(x=times_min, y=np.repeat(y_center, N), orientation='h', side='positive', width=global_cloud_width, scalemode='count', line_color=c_base, fillcolor=c_fill, showlegend=False, points=False, box_visible=False, hovertemplate=hover))
        fig_A.add_trace(go.Box(x=times_min, y=np.repeat(box_center, N), orientation='h', width=global_box_width, line_color=c_base, fillcolor=c_box, boxpoints='outliers', marker=dict(color=c_outlier, size=4, opacity=0.8), showlegend=False, hoveron='points', hovertemplate="<b>Outlier</b><br>Actual Arrival: %{x:+.1f} min<extra></extra>"))
    ...
    for line_data in mode_b_lines:
        ...
        fig_B.add_trace(go.Scattergl(x=line_data['x'], y=line_data['y'], mode='lines+markers', line=dict(width=0.3), marker=dict(size=1.5), opacity=1.0, connectgaps=False, name=line_data['name'], text=line_data['speed'], customdata=cd, hovertemplate="<b>Absolute Time:</b> %{customdata[3]}<br><b>Relative Arrival:</b> %{x:+.1f} min<br><b>Track Distance:</b> %{y:.2f} km<br><b>Speed:</b> %{text:.1f} km/h<br><br><b>Date:</b> %{customdata[0]}<br><b>Trip ID:</b> %{customdata[2]}<br><b>GPS:</b> %{customdata[4]:.5f}, %{customdata[5]:.5f}<br><extra></extra>"))

    sched_trace = go.Scattergl(x=st_filtered['relative_sec'] / 60.0, y=st_filtered['shape_dist_traveled'], mode='lines+markers', line=dict(color='#000000', width=1.4), marker=dict(symbol='circle', size=4.5, color='#000000'), name="Scheduled Baseline", hovertemplate="<b>%{customdata}</b><br>Scheduled Profile: %{text}<extra></extra>", text=[format_relative_time(s) for s in st_filtered['relative_sec']], customdata=st_filtered['stop_name'])
    fig_A.add_trace(sched_trace)
    fig_B.add_trace(sched_trace)

    common_layout = dict(yaxis_title="Official Track Distance (km) & Stops", template="plotly_white", autosize=True, yaxis=dict(range=[st_filtered['shape_dist_traveled'].min() - 0.02, st_filtered['shape_dist_traveled'].max() + 0.05], tickmode='array', tickvals=st_filtered['shape_dist_traveled'], ticktext=y_tick_texts, gridcolor='lightgray', automargin=True, zeroline=False, tickfont=dict(size=10)), xaxis=dict(zeroline=False, showline=True, showgrid=True, gridcolor='lightgray', dtick=5, ticksuffix=" min", automargin=True, tickfont=dict(size=11)), margin=dict(l=10, r=20, t=60, b=10), legend=dict(font=dict(size=10), itemsizing='constant'))
...
def filter_dialog(available_routes, parquet_path, trips, stop_times, stops, shapes):
    ...
    with col_a:
        st.subheader("1. Route Configuration")
                ...
                if valid_trips.empty:
                    ...
                    if not sig_ui_list:
                        ...
                        st.session_state.stage2_vars = {
                            'trip_start_dict': trip_start_dict, 'filter_start_sec': filter_start_sec, 'filter_end_sec': filter_end_sec,
                            'time_mode': time_mode, 'force_t0': force_t0, 'day_type': day_type, 'time_range_str': f"{start_time_input}-{end_time_input}"
                        }
        
        st.divider()
        if st.session_state.signatures_loaded:
            st.subheader("3. Finalize & Apply")
